...
function wrapper(plugin_info) {
    ...
    window.plugin.jais.createHomogenousFieldDialog = function() {
        ...
        '<button class=\'jais-button ui-dialog-buttonset\' onclick=\'window.plugin.jais.getHomogenousFieldData()\' role=\'button\'>Generate</button>';
        dialog({
            html: html,
            width: 500,
    ...
    window.plugin.jais.getHomogenousFieldData = function() {
        var self = window.plugin.jais;

        var xhr = new XMLHttpRequest();
        var user = document.getElementById('loginuser').value;
        var password = document.getElementById('loginpassword').value;
        var data = self.getHomogenousJson(user, password);
        ...
        xhr.onreadystatechange = function() {
            if(this.readyState == XMLHttpRequest.DONE && this.status == 200) {
                window.plugin.jais.processHomogenousData(this.responseText);
            }
        }
        xhr.send(JSON.stringify(data));
    }

    window.plugin.jais.getHomogenousJson = function(user, password) {
        var self = window.plugin.jais,
    ...
    window.plugin.jais.processHomogenousData = function(response) {
        try {
    ...
    window.plugin.jais.getPortalsFromPolygon = function() {
        ...
        for (var i = 0; i < layers.length; i++) {
            ...
            for(var points = 0; points < drawnItem._latlngs.length; points++) {
                drawItemArray.push([drawnItem.getLatLngs()[points].lat, drawnItem.getLatLngs()[points].lng]);
            }
            for(var p = 0; p < Object.keys(window.portals).length; p++) {
                var thePortal = window.portals[Object.keys(window.portals)[p]];
                if(inside([thePortal.getLatLng().lat, thePortal.getLatLng().lng], drawItemArray)) {
                    portalArray.push(thePortal);
                } else {
                    for(var j = 0; j < drawItemArray.length; j++) {
                        if(thePortal.getLatLng().lat === drawItemArray[j][0] && thePortal.getLatLng().lng === drawItemArray[j][1]) {
                            portalArray.push(thePortal);
                        }
                    }
                }
