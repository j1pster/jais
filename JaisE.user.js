...
function wrapper(plugin_info) {
    ...
    window.plugin.jaisE.boot = function() {
        $('#toolbox').append('<a onclick="window.plugin.jaisE.openJaisDialog()" title=>Jais</a>');
        window.plugin.jaisE.justAnotherPortalArray = [];
        window.plugin.jaisE.addEventListeners();
        window.plugin.jaisE.jsonOutput = {
            "portals": window.plugin.jaisE.justAnotherPortalArray
        };
        window.plugin.jaisE.drawToolsDataLoaded = false;
        window.plugin.jaisE.links = [];
        window.plugin.jaisE.link = {
            init: function(o, d, color, order) {
                this.from = Object.create(window.plugin.jaisE.portal);
                this.from.init(o);
                this.to = Object.create(window.plugin.jaisE.portal);
                this.to.init(d);
                this.order = order !== undefined ? order : window.plugin.jaisE.links.length;
    ...
    window.plugin.jaisE.addEventListeners = function() {
        var self = window.plugin.jaisE;
        window.addHook('mapDataRefreshEnd', function() {
            if(!self.drawToolsDataLoaded) {
                window.addHook('pluginDrawTools', self.drawToolsHandler);
                if(window.plugin.drawTools) {
                    window.plugin.jaisE.load();
    ...
    window.plugin.jaisE.openJaisDialog = function() {
        ...
        if(window.plugin.drawTools) {
            if(Object.keys(window.plugin.drawTools.drawnItems._layers).length === 0) {
                ...
                '<button type=\'button\' class=\'jais-button ui-dialog-buttonset\' onclick=\'window.plugin.jaisE.exportPortalsFromPolygon();\' role=\'button\'>Export</button>' +
                '<button type=\'button\' class=\'jais-button ui-dialog-buttonset\' onclick=\'window.plugin.jaisE.bookMarkPortalsFromPolygon();\' role=\'button\'>Bookmark</button>';
                html += '<div id=\'textareadiv\' class=\'ui-dialog-content\'><textarea readonly onclick="$(\'#textareadiv textarea\').select();">'+ JSON.stringify(window.plugin.jaisE.justAnotherPortalArray) +'</textarea>' + 
                ...
                '<button type=\'button\' class=\'jais-button ui-dialog-buttonset\' onclick=\'window.plugin.jaisE.clearPortals();\' role=\'button\'>Clear</button>' +
    ...
    window.plugin.jaisE.createCountDialog = function(thePortals, portalArray, linkCount, fieldCount, portalCount, resoCount, potentialAP) {
    ...
    window.plugin.jaisE.linkDialog = function() {
        self = window.plugin.jaisE;
        ...
        if(self.links.length === 0) {
            ...
            html += "<button type=\'button\' id=\'saveLinks\' class=\'jais-button ui-dialog-buttonset\' onclick=\'window.plugin.jaisE.saveLinkOrder();\' role=\'button\'>Save</button>" +
            "<button type=\'button\' id=\'clearLinks\' class=\'jais-button ui-dialog-buttonset\' onclick=\'window.plugin.jaisE.clearLinks();\' role=\'button\'>Clear</button>";
            html += "<div id=\'tableWrapper\'><div id=\'linkAnchor\'>" + self.linkHTML() + "</div><div id=\'portalAnchor\'>" + self.portalsInPlan() + "</div></div>"; 
        }
        html += "<button id=\'exportToCsv\' class=\'jais-button2 ui-dialog-buttonset\' onClick=\'window.plugin.jaisE.exportLinksToCSV(); \' role=\'button\'>Export to CSV</button>" + 
        "<button class=\'jais-button2 ui-dialog-buttonset\' onClick=\'window.plugin.jaisE.exportPortalsInPlan();\' role=\'button\'>Export Portals</button></div>";
        dialog({
            html: html,
            width: 500,
    ...
    window.plugin.jaisE.linkHTML = function() {
        ...
        for(var i = 0; i < window.plugin.jaisE.links.length; i++) {
            var link = window.plugin.jaisE.links[i];
            link.idx = i;
            ...
            "<td><button class=\'deleteButton\' onclick=\'window.plugin.jaisE.deleteLink(" + i + ");\' role=\'button\'></button></td>" +
            "<td><button class=\'swapButton\' onclick=\'window.plugin.jaisE.swapLink(" + i + ");\' role=\'button\'></button></td></tr>";
        }
        html += "</tbody></table>";
        return html;
    };
    
    window.plugin.jaisE.portalHTML = function(portals) {
    ...
    window.plugin.jaisE.portalAlreadyInOp = function(portalsInOp, thePortal) {
    ...
    window.plugin.jaisE.portalAlreadyInPlan = function(linksInPlan, thePortal, currentLink) {
    ...
    window.plugin.jaisE.linkAlreadyInOp = function(linksInOp, theLink) {
    ...
    window.plugin.jaisE.arrayContains = function(theArray, obj){
    ...
    window.plugin.jaisE.checkForDrawTools = function() {
    ...
    window.plugin.jaisE.GetInfoAndAddToArray = function(portal) {
        var currentPortal = {
        ...
        window.plugin.jaisE.justAnotherPortalArray.push(currentPortal);
    };
    
    window.plugin.jaisE.orderSort = function(a, b) {
    ...
    window.plugin.jaisE.saveLinkOrder = function() {
        var self = window.plugin.jaisE;
    ...
    window.plugin.jaisE.deleteLink = function(idx) {
        var self = window.plugin.jaisE;
    ...
    window.plugin.jaisE.swapLink = function(idx) {
        var self = window.plugin.jaisE;
        self.links[idx].swapLink();
        self.saveLinkOrder();
    };
    
    window.plugin.jaisE.clearLinks = function() {
        var table = document.getElementById("linkTable");
        table.parentElement.removeChild(table);
        window.plugin.jaisE.links = [];
    };
    
    window.plugin.jaisE.convertLayerToLinks = function(layer) {
        console.log(layer);
        var pi = [];
        var self = window.plugin.jaisE;
    ...
    window.plugin.jaisE.save = function() {
        localStorage['plugin-jais-links'] = JSON.stringify(window.plugin.jaisE.links);
    };
    
    window.plugin.jaisE.load = function() {
    ...
    window.plugin.jaisE.portalsInPlan = function() {
        var self = window.plugin.jaisE; 
        var thePortals = {}
        var portalArray = [];
    ...
    window.plugin.jaisE.countPortals = function() {
        var self = window.plugin.jaisE;
    ...
    window.plugin.jaisE.clearPortals = function() {
        window.plugin.jaisE.justAnotherPortalArray = [];
        $(".ui-dialog-jais-export textarea").val(JSON.stringify(window.plugin.jaisE.justAnotherPortalArray));
        return;
    };
    
    window.plugin.jaisE.exportPortalsFromPolygon = function() {
        $('#textareadiv').show();
        var self = window.plugin.jaisE;
        if(self.checkForDrawTools()) {
            var thePortals = self.getPortalsFromPolygon();
    ...
    window.plugin.jaisE.closeTextArea = function() {
        $('#textareadiv').hide();
        return;
    };
    
    window.plugin.jaisE.exportLinksToCSV = function() {
        var self = window.plugin.jaisE;
        var headers = {
            column1: "From Portal",
    ...
    window.plugin.jaisE.exportPortalsInPlan = function() {
        var self = window.plugin.jaisE;
        var headers = {
            column1: "Name",
            column2: "Keys Needed", 
    ...
    window.plugin.jaisE.exportPortalsInPolygon = function() {
        var self = window.plugin.jaisE;
        var headers = {
            column1: "Name", 
            column2: "Latitude", 
    ...
    window.plugin.jaisE.buildCSV = function(headers, data, filename) {
    ...
    window.plugin.jaisE.bookMarkPortalsFromPolygon = function() {
        var self = window.plugin.jaisE;
        var html = "";
    ...
    window.plugin.jaisE.PortalNotYetBookmarked = function(portal) {
        self = window.plugin.jaisE;
    ...
    window.plugin.jaisE.getFieldsFromPolygon = function(thePortals) {
    ...
    window.plugin.jaisE.getLinksFromPolygon = function(thePortals) {
        var linkCount = [0, 0, 0],
        linkArray = [],
        self = window.plugin.jaisE;
        var theLink;
    ...
    window.plugin.jaisE.getPortalsFromPolygon = function() {
        var portalCount = 0,
        portalArray = [],
        self = window.plugin.jaisE;
        var drawItemArray = [];
    ...
    window.plugin.jaisE.drawToolsHandler = function(e) {
        var self = window.plugin.jaisE;
        console.log(e);
    ...
    window.plugin.jaisE.layerCreatedHandler = function(e) {
        var self = window.plugin.jaisE;
        var layer = e.layer;
        self.convertLayerToLinks(layer);
        window.plugin.jaisE.save();
    };
    
    window.plugin.jaisE.layerDeletedHandler = function(layers) {
